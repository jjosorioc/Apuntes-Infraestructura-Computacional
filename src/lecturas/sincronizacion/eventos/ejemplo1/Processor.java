    ...
    public void produce() throws InterruptedException {
        ...
        while (true) {
            synchronized (lock) {
                ...
                while (list.size() == LIMIT) { // si el tamaño de la lista es igual al limite,
                                               // espera a que se produzca un evento de consumir
                    lock.wait();
                }
    ...
    public void consume() throws InterruptedException {
        ...
        while (true) {
            synchronized (lock) {
                ...
                while (list.size() == 0) { // si el thread inica en este proceso y no hay nada en la
                                           // lista, espera a que se produzca algo
                    lock.wait();
                }
